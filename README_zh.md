...
`iiilab.com视频解析接口的逆向爬虫`

[English](README.md)

## 简介

本仓库是对[iiilab.com](https://www.iiilab.com/)的视频解析接口的逆向爬虫，该网站支持多个平台的视频链接解析下载。

更新：现在部分平台解析的被重定向到[snapany.com](https://snapany.com/)，旧版接口使用前端参数加密，新版接口使用前端签名校验。
旧版接口的爬虫脚本在`old-version-2024`中。

目前脚本中仅包含了对TikTok/抖音、YouTube、B站、Facebook、Instagram、X/Twitter平台的支持。如果只有单个视频的解析下载需求，可以直接使用原网站：

- [snapany.com（TikTok/抖音、YouTube、B站）](https://snapany.com/zh/)
...
如果请求失败，`get_resource`会返回`None`并输出错误详情；否则返回解析后的JSON响应数据。

如果希望支持更多平台，只需要参考`iiilab.py`添加对应的`SITE`常量（前提是原网站支持对应平台的解析）。
